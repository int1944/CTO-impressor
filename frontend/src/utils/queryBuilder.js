 ...
 */
export function buildQuery(currentQuery, suggestion, entityType, cursorPosition = null) {
  ...
  switch (entityType) {
    case 'from':
      ...
      if (queryLower.includes(' from ')) {
        ...
        const fromMatch = query.match(/\bfrom\s+([^\s]+(?:\s+[^\s]+)*?)(?=\s+(?:to|on|for|in|$))/i);
        if (fromMatch) {
      ...
      if (!queryLower.includes('from')) {
        ...
        if (intentMatch) {
          ...
          return query.substring(0, pos) + ` from ${entity}` + query.substring(pos);
        }
      }
      break;
    ...
    case 'to':
      ...
      if (queryLower.includes(' to ')) {
        const toMatch = query.match(/\bto\s+([^\s]+(?:\s+[^\s]+)*?)(?=\s+(?:on|for|in|from|$))/i);
        if (toMatch) {
      ...
      if (!queryLower.includes(' to ')) {
        ...
        if (fromMatch) {
          ...
          return query.substring(0, pos) + ` to ${entity}` + query.substring(pos);
        }
      }
      break;
    ...
    case 'date':
      ...
      if (queryLower.includes(' on ')) {
        const onMatch = query.match(/\bon\s+([^\s]+(?:\s+[^\s]+)*?)(?=\s+(?:for|in|from|to|$))/i);
        if (onMatch) {
