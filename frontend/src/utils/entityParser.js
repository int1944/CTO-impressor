...
export function parseEntities(query) {
  ...
  const datePatterns = [
    ...
    { pattern: /\b(sunday|monday|tuesday|wednesday|thursday|friday|saturday)\b/gi, type: 'date' },
    { pattern: /\b(january|february|march|april|may|june|july|august|september|october|november|december)\b/gi, type: 'date' },
    { pattern: /\b\d{1,2}\s+(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\b/gi, type: 'date' },
    { pattern: /\b\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}\b/g, type: 'date' },
  ];

  // City/location patterns (common cities and patterns)
  const cityPatterns = [
    { pattern: /\b(from|to)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b/g, type: 'city' },
    // Match common city abbreviations
    { pattern: /\b(NYC|SFO|LAX|JFK|LHR|CDG|DXB|BOM|DEL|BLR|MAA|CCU|HYD)\b/gi, type: 'city' },
  ];

  // Time patterns
  const timePatterns = [
    { pattern: /\b(morning|afternoon|evening|night|midnight|noon)\b/gi, type: 'time' },
